...
- Root Cause: The server is currently communicating via HTTP (Port 80). Traffic sent over HTTP is unencrypted (plain text), which poses a risk of Man-in-the-middle (MITM) attacks where an adversary could intercept credentials.
