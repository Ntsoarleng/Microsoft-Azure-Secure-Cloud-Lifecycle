...
Firstly, I checked the IP Address of the Cloud Shell and it was still the same as the one from the previous phase. This meant that the session had not expired and changed the IP Address. Therefore, there was no need to change the NSG rules as the IP Address has remained the same from the last time the NSG rules were updated.
